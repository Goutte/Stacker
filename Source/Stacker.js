...
var Stacker = new Class({
    ...
    initialize: function(motherDiv, options) {
        this.setOptions (options);
        this.motherDiv  = document.id(motherDiv); // Background image, fully opaque

        this.defaultImage = this.normalizeImagePath(this.motherDiv.getStyle('background-image'));
    ...
    loadGallery: function (gallery, htmlContent) {
        ...
        this.gallery = gallery.map(function(item,index){return this.options.imagePath+item}.bind(this));
    ...
    addStackedElement: function (imagePath, htmlContent) {
        var element = this.motherDiv.clone(false,false).setStyle('opacity',0).set('html', htmlContent);
        element.setStyle('z-index', this.motherDiv.getStyle('z-index').toInt() + this.imageElementsStack.length + 1);
        element.setStyle('background-image', 'url('+imagePath+')').inject(this.motherDiv, 'after');
